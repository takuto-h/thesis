...
\begin{abstract}
%[概要（400字程度）]
従来のクラスベース・オブジェクトシステムは，クラスにメソッドを格納するモデルと総称関数にメソッドを格納する
モデルのどちらかにモデル化できる．
本論文はそのどちらでもない，環境にメソッドを直接格納するモデルに基づく新しいオブジェクトシステムを
提案する．
このモデルは変数が変数名をキーとして環境に格納されているととらえるのと同じように，メソッドがクラス名と
メソッド名の組をキーとして環境に格納されているととらえる．
これにより変数に対して行える操作をメソッドに対しても自然に意味付けできる．
そのようなメソッドの操作が実際のプログラミングにおいて有用であることを示すため，提案するオブジェクト
システムを搭載する独自言語Suzuの処理系を実装した．
Suzuではそういった操作を活用して簡潔で可読性の高い言語内DSL（Domain Specific Language）を
容易に構築できる．
提案するオブジェクトシステムと類似する既存の概念との関連についても議論する．
\end{abstract}
